...
// Import Chat types from types/chat.ts
...
class LocalMessage implements Message {
  ...
  constructor (type: MessageType, payload: Message['payload']) {
    ...
    if (!this.payload.messageId) {
      this.payload.messageId = `${Date.now()}${Math.floor(Math.random() * 10_000)}`
    }
...
export { ContentType, LocalMessage, MessagePing, MessagePong, MessageStatus, MessageText, MessageType }
export { type Chat, type ChatAreaProps, type ChatItemProps, type ChatListProps, type ChatType } from '@/types/chat'
